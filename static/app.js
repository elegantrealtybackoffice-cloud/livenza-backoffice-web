...
(()=>{
  ...
  cameraFallback?.addEventListener('change',()=>{const file=cameraFallback.files?.[0];if(file&&previewFile(file)){cameraBlob=file;cameraFileName=file.name||`livenza-camera-${Date.now()}.jpg`;avatarSourceFile=file;if(input)input.value=''}});
  captureButton?.addEventListener('click',()=>{if(!cameraVideo?.videoWidth||!cameraCanvas){showState('error','Camera frame is not ready','Wait until the camera preview appears, then capture again.');return}const vw=cameraVideo.videoWidth,vh=cameraVideo.videoHeight;const max=1600,scale=Math.min(1,max/Math.max(vw,vh));cameraCanvas.width=Math.round(vw*scale);cameraCanvas.height=Math.round(vh*scale);const ctx=cameraCanvas.getContext('2d');ctx.save();ctx.translate(cameraCanvas.width,0);ctx.scale(-1,1);ctx.drawImage(cameraVideo,0,0,vw,vh,0,0,cameraCanvas.width,cameraCanvas.height);ctx.restore();cameraCanvas.toBlob(blob=>{if(!blob){showState('error','Could not capture photo','Use Camera File Picker as a fallback.');return}cameraBlob=blob;cameraFileName=`livenza-selfie-${Date.now()}.jpg`;avatarSourceFile=new File([blob],cameraFileName,{type:'image/jpeg'});previewFile(avatarSourceFile);if(input)input.value='';stopCamera(true)},'image/jpeg',.94)});
  input?.addEventListener('change',()=>{const file=input.files?.[0];if(!file)return;cameraBlob=null;cameraFileName='';avatarSourceFile=file;previewFile(file)});
  form.addEventListener('submit',async event=>{
    event.preventDefault();if(busy)return;avatarSourceFile=avatarSourceFile||input.files?.[0]||null;const hasSavedPhoto=form.dataset.hasSavedPhoto==='1';if(!avatarSourceFile&&!hasSavedPhoto){showState('error','Choose or capture a photo','You can use almost any JPG/PNG/WebP/HEIC photo or take a laptop webcam photo.');return}
    // Build the payload before disabling the visible file input. Disabled controls are omitted by FormData in every major browser.
    const fd=new FormData(form);
    if(avatarSourceFile)fd.set('avatar_photo',avatarSourceFile,avatarSourceFile.name||cameraFileName||`livenza-avatar-${Date.now()}.jpg`);
    else fd.set('regenerate_avatar','1');
    setBusy(true);showState('loading','Creating your live avatar…',avatarSourceFile?'Using the complete source image, multiple identity references and a dependable local fallback.':'Reusing your saved profile photo with the connected AI image engine.');
