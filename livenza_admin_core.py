...
def production_config_errors(env) -> list[str]:
    ...
    if get('RAZORPAY_TEST_STUB','0')=='1':
        errors.append('RAZORPAY_TEST_STUB cannot be enabled in production.')
    key_id=get('RAZORPAY_KEY_ID'); key_secret=get('RAZORPAY_KEY_SECRET'); webhook=get('RAZORPAY_WEBHOOK_SECRET')
    if not key_id.startswith('rzp_live_') or not key_secret or not webhook:
        errors.append('Razorpay live key, secret and webhook secret are required in production.')
