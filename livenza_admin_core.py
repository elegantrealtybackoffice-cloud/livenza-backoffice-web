...
def production_config_errors(env) -> list[str]:
    ...
    if environment not in {'production','prod'}:
        return []
    errors=[]
    consumer_enabled=get('LIVENZA_CONSUMER_PLATFORM_ENABLED','0').lower() in {'1','true','yes','on'}
    ...
    if consumer_enabled and not (get('SUPABASE_URL').startswith('https://') and get('SUPABASE_SERVICE_ROLE_KEY') and get('LIVENZA_PRIVATE_BUCKET')):
